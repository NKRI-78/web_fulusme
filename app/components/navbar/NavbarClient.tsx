...
import { useUser } from "@/app/features/auth/hook/useUser";
import { logoutUser } from "@/app/features/auth/services/auth-service";
...
export default function NavbarClient({
  ...
  const getAvatar = (): string => {
    if (user) {
      ...
      if (user.selfie !== "-") return user.selfie;
    }
    return "/images/default-image.png";
  };

  const getUsername = (): string => {
    if (loadingUser || !user?.fullname) {
      return "Memuat..";
    }
            ...
            {session !== null ? (
              ...
              <>
                ...
                <div className="flex items-center gap-4">
                  {session.role !== "user" ? (
                    <Link href={"/profile"}>
                      <div className="hidden md:flex items-center gap-x-3 px-4 py-2 bg-[#0c484d] rounded-full hover:bg-[#0b363a] transition-colors duration-500">
                        <p className="text-white text-sm">{getUsername()}</p>
                        <img
                          src={getAvatar()}
                          alt="Foto Profile"
                          width={28}
                  ...
                  <NotifIcon
                    badgeCount={0}
                    className={isSticky ? `text-primary` : `text-on-primary`}
                  />
                ...
                <div
                  className={`fixed top-0 right-0 h-full w-64 bg-primary z-40 p-6
                    ...
                    <div
                      className={`px-5 py-[6px] rounded-md ${
                        isSticky
                          ...
                          : `border-2 border-on-primary text-on-primary`
                      } cursor-pointer`}
                      onClick={() => {
                        setStep("register");
                        // setStep("role");
                ...
                >
                  ...
                  <ul className="flex flex-col gap-6 text-white text-sm font-semibold pt-16">
                    ...
                    <li className="w-fit" onClick={toggleMenu}>
                      <Link href="/login">
                        <div
                          className={`px-8 py-2 rounded-md bg-white text-primary`}
                        >
                          Masuk
                        </div>
                      </Link>
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        sticky ? "bg-on-primary text-primary shadow-md" : "bg-primary"
      }`}
    >
      {children}
    </div>
